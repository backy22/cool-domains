...
contract Domains is ERC721URIStorage {
  ...
  string public tld; // TDL ... top-level domain. なにで終わるか ex .ninja

  // storing our NFT images on chain as SVGs
  ...
  mapping(string => address) public domains; // state variable ... stored permanently in the contract’s storage
  ...
  function register(string calldata name) public payable {
    ...
    string memory _name = string(abi.encodePacked(name, ".", tld)); // stringをcombineするにはencodePackedを使う必要
  ...
  function getAddress(string calldata name) public view returns (address) { //  calldata ... nameがstoreされる場所を指定
